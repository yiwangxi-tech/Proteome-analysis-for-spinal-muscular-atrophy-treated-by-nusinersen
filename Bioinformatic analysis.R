...
perform_de_analysis <- function() {
     ...
     before_group_2 <- factor(c(rep("SMA1", 1), rep("SMA2", 6)))
     ...
     before_contrast_3 <- makeContrasts(SMA1 - SMA3, levels = before_design_3)
     ...
     before_samples_4 <- c(SMA1_after, SMA2_after)
     before_data_4 <- log2_data[, before_samples_4]
     before_group_4 <- factor(c(rep("SMA1", 1), rep("SMA2", 3)))
     before_design_4 <- model.matrix(~0 + before_group_4)
     colnames(before_design_4) <- c("SMA1", "SMA2")
     before_fit_4 <- lmFit(before_data_4, before_design_4)
     before_contrast_4 <- makeContrasts(SMA1 - SMA2, levels = before_design_4)
     before_fit2_4 <- contrasts.fit(before_fit_4, before_contrast_4)
     before_fit2_4 <- eBayes(before_fit2_4)
     results_list$before_results_4 <- topTable(before_fit2_4, n = Inf)
     results_list$before_results$Protein_4 <- rownames(results_list$before_results_4)
     
     # Analysis 5: SMA-1 vs SMA-3 (After treatment)
     before_samples_5 <- c(SMA1_after, SMA3_after)
     before_data_5 <- log2_data[, before_samples_5]
     before_group_5 <- factor(c(rep("SMA1", 1), rep("SMA3", 6)))
     before_design_5 <- model.matrix(~0 + before_group_5)
     colnames(before_design_5) <- c("SMA1", "SMA3")
     before_fit_5 <- lmFit(before_data_5, before_design_5)
     before_contrast_5 <- makeContrasts(SMA1 - SMA3, levels = before_design_5)
     before_fit2_5 <- contrasts.fit(before_fit_5, before_contrast_5)
     before_fit2_5 <- eBayes(before_fit2_5)
     results_list$before_results_5 <- topTable(before_fit2_5, n = Inf)
     results_list$before_results$Protein_5 <- rownames(results_list$before_results_5)
     
     # Analysis 6: SMA-2 vs SMA-3 (After treatment)
     before_samples_6 <- c(SMA2_after, SMA3_after)
     before_data_6 <- log2_data[, before_samples_6]
     before_group_6 <- factor(c(rep("SMA2", 3), rep("SMA3", 6)))
     before_design_6 <- model.matrix(~0 + before_group_6)
     colnames(before_design_6) <- c("SMA2", "SMA3")
     before_fit_6 <- lmFit(before_data_6, before_design_6)
     before_contrast_6 <- makeContrasts(SMA2 - SMA3, levels = before_design_6)
     before_fit2_6 <- contrasts.fit(before_fit_6, before_contrast_6)
     before_fit2_6 <- eBayes(before_fit2_6)
     results_list$before_results_5 <- topTable(before_fit2_6, n = Inf)
     results_list$before_results$Protein_6 <- rownames(results_list$before_results_6)
     
     # Analysis 7: Before vs After (SMA-1)
     before_samples_7 <- c(SMA1_before, SMA1_after)
     before_data_7 <- log2_data[, before_samples_7]
     before_group_7 <- factor(c(rep("Before", 1), rep("After", 1)))
     before_design_7 <- model.matrix(~0 + before_group_7)
     colnames(before_design_7) <- c("Before", " After")
     before_fit_7 <- lmFit(before_data_7, before_design_7)
     before_contrast_7 <- makeContrasts(After - Before, levels = before_design_7)
     before_fit2_7 <- contrasts.fit(before_fit_7, before_contrast_7)
     before_fit2_7 <- eBayes(before_fit2_7)
     results_list$before_results_7 <- topTable(before_fit2_7, n = Inf)
     results_list$before_results$Protein_7 <- rownames(results_list$before_results_7)
     
     # Analysis 8: Before vs After (SMA-2)
     before_samples_8 <- c(SMA2_before, SMA2_after)
     before_data_8 <- log2_data[, before_samples_8]
     before_group_8 <- factor(c(rep("Before", 3), rep("After", 3)))
     before_design_8 <- model.matrix(~0 + before_group_8)
     colnames(before_design_8) <- c("Before", " After")
     before_fit_8 <- lmFit(before_data_8, before_design_8)
     before_contrast_8 <- makeContrasts(After - Before, levels = before_design_8)
     before_fit2_8 <- contrasts.fit(before_fit_8, before_contrast_8)
     before_fit2_8 <- eBayes(before_fit2_8)
     results_list$before_results_8 <- topTable(before_fit2_8, n = Inf)
     results_list$before_results$Protein_8 <- rownames(results_list$before_results_8)
     
     # Analysis 9: Before vs After (SMA-3)
     before_samples_9 <- c(SMA3_before, SMA3_after)
     before_data_9 <- log2_data[, before_samples_9]
     before_group_9 <- factor(c(rep("Before", 6), rep("After", 6)))
     before_design_9 <- model.matrix(~0 + before_group_9)
     colnames(before_design_9) <- c("Before", " After")
     before_fit_9 <- lmFit(before_data_8, before_design_9)
     before_contrast_9 <- makeContrasts(After - Before, levels = before_design_9)
     before_fit2_9 <- contrasts.fit(before_fit_9, before_contrast_9)
     before_fit2_9 <- eBayes(before_fit2_9)
     results_list$before_results_9 <- topTable(before_fit2_9, n = Inf)
     results_list$before_results$Protein_9 <- rownames(results_list$before_results_9)
